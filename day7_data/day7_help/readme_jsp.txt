...
2.8 page --- translated page class instance 
(created for 'this' JSP )
...
<%= session.setAttribute("nm",val) %> --compile time error
<%= session.getId() %>
...
2. session --- boolean attribute. default value=true.
...
3. Properties of JBs --- private, non-static , non-transient Data members  --- equivalent to request params sent by clnt.(Prop names MUST match with req params for easy usage : meaning 
...
2. Use taglib directive to include JSTL tag library into the  JSP page.
tag=action
...
eg ---How  to import JSTL supplied core tag lib?
...
3.1 eg 
<c:set var="abc" value="${param.f1}"  scope="request" />
---WC 
request.setAttribute("abc",request.getParameter("f1"))
...
3.2 JB --- ShopBean (session scope) -- property -- 
private AL<Category> categories; --g & s

In JSP 
${sessionScope.shop.categories}
